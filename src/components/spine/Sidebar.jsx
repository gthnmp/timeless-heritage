...
const TableOfContent = ({ handleClick, sidebarVisible }) => (
  <div id = "menu" className={`h-full w-full relative transition-all duration-1500 easeInBack px-10 lg:pl-32 py-16 bg-primary-color noto-serif text-5xl lg:text-9xl font-medium uppercase tracking-tighter flex flex-col lg:flex-row transform drop-shadow-2xl ${sidebarVisible ? "" : "delay-500"}`}
    ...
    <div className="w-full relative flex justify-end items-end ">
      <button className={`w-52 h-52 rounded-full bg-secondary-color hidden lg:block absolute top-0 transition-tranform duration-1000 ${sidebarVisible ? "opacity-100 scale-75 delay-500" : "opacity-0 scale-0"}`} onClick={handleClick}>
