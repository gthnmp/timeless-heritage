...
export default function Sidebar() {
  ...
  return (
    <header className={`fixed top-0 w-full h-screen z-20`}>
      ...
      <div id = "menu" className={`h-full w-full relative transition-all duration-1500 easeInBack px-10 lg:pl-32 py-16 bg-yellow-500 noto-serif text-5xl lg:text-9xl font-medium uppercase tracking-tighter flex flex-col lg:flex-row transform ${sidebarVisible ? "" : "delay-500"}`}
        ...
        <ul className="flex flex-col h-full gap-10 lg:gap-0 justify-center lg:justify-between">
          ...
          <li className="overflow-hidden">
            <a href="/" className="relative text-black">
              <h1 id = "nav-button" className={`transition-transform w-max ease-in-out duration-1000 ${sidebarVisible ? 'translate-y-0 delay-1200' : 'translate-y-full'}`}>Articles</h1>
            </a>
          </li>
          <li className="overflow-hidden">
            <a href = "/" className="relative text-black">
